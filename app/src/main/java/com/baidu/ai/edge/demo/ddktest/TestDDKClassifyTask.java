...
public class TestDDKClassifyTask extends BaseTestTask<Void, CharSequence, CharSequence> {
    ...
    protected CharSequence doInBackground(Void... voids) {
        ...
        try {
            for (int i = 0; i < NUM_OF_RUNS; i++) {
                ...
                DDKConfig config = new DDKConfig(context.getAssets(), "ddk", getDDKType());
    ...
    private int getDDKType() throws Exception {
        if (Build.HARDWARE.contains("kirin970")) {
            return Consts.TYPE_DDK150;
        }
        if (Build.HARDWARE.contains("kirin980")) {
            return Consts.TYPE_DDK200;
        }
        throw new Exception("Build.HARDWARE does not support the DDK test. Build.HARDWARE is: "
                + Build.HARDWARE);
    }
}
